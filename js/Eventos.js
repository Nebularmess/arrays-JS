...
if (eleccionTarj == 1) {
    ...
    denuncia ()
    if (bandera == true) {
        alert ('Su tarjeta ha sido denunciada, pongase en contacto con su banco para recibir información respectiva a la nueva, proseguimos con su tarjeta de credito')
        tarjeta.className = 'card-registrada-denunciada'
    }
    if (bandera == false) {
        alert ('Denuncia desestimada, proseguimos con su tarjeta de credito')
    }
...
if (eleccionTarj == 0) {
    ...
    if (bandera == true) {
        ...
        eleccion = prompt('Confirma la denuncia de la tarjeta de credito perteneciente al banco ' + tarjetaCredito.banco + ' a nombre de ' + tarjetaCredito.nombre + '? Ingrese 1 para confirmar y 0 para desestimar')
        if (eleccion != 1 && eleccion != 0) {
            youShallNotPass ()
        }
        if (eleccion == 1) {
